package com.mol.purchase.service.dingding.login;
...
import com.mol.purchase.util.DDEntityChangeUtil;
...
public class LoginService {
    ...
    public List<DDUser> getDeptUsersByDeptId(Long deptId){
        ...
        try {
            ...
            request.setSize(10L);
            request.setHttpMethod("GET");
            OapiUserListbypageResponse response = client.execute(request, tokenService.getToken());
            if(response == null){
    ...
    public DDDept getDeptDetail(Long deptId){
        ...
        request.setHttpMethod("GET");
        try {
            OapiDepartmentGetResponse response = client.execute(request, tokenService.getToken());
